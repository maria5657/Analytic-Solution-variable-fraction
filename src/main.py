...
from p_universal_alpha import get_p_universal, calc_alpha
...
numerical_constant_coef_sol = get_numerical_solution(
    s_val=s_value, constants_values=constants_vals, alpha=-2, coef_type=CoefType.constant
)
...
# Тест-сверка аналитической формулы с непостоянными коэффициентами и численным решением alpha=-2

print('Тест-сверка аналитической формулы с непостоянными коэффициентами и численным решением alpha=-2')

analytic_nonconstant_coef_sol = get_p_alpha_minus_2(x_D=0, val_s=s_value, vals=constants_vals)
numerical_nonconstant_coef_sol = get_numerical_solution(
    s_val=s_value, constants_values=constants_vals, alpha=-2, coef_type=CoefType.nonconstant
)
...
t_moments = np.exp(np.linspace(np.log(10 - 4), np.log(1000 * 3600), num=10))
...
plt.xscale('log')
plt.grid(True)
plt.savefig(str(res_path / 'res2.png'), dpi=200)
...
constants_vals.val_n = 0.25  # 0.25, 0.75

analytic_coef_sol = get_p_universal(x_D=0, s=s_value, vals=constants_vals)
numerical_coef_sol = get_numerical_solution(
    s_val=s_value, constants_values=constants_vals, alpha=calc_alpha(constants_vals), coef_type=CoefType.nonconstant
)

print(
    f'Аналитическая формула: {"%.2e" % analytic_coef_sol}',
    f'Численная формула: {"%.2e" % numerical_coef_sol}',
    f'Разница: {"%.2e" % abs(analytic_coef_sol - numerical_coef_sol)}',
    sep='\n',
    end='\n\n',
)
